...
export async function GET(req: NextRequest) {
  ...
  const { allowed, reason } = await checkAccountLimit(userId, "facebook");
  if (!allowed) {
  ...
  const successRedirect = encodeURIComponent(
    `${baseUrl}/api/connect/callback?platform=facebook`
  );
  const failureRedirect = encodeURIComponent(
    `${baseUrl}/dashboard/settings?error=facebook_denied`
  );

  const authUrl =
    `https://api.unified.to/unified/integration/auth/${workspaceId}/facebook` +
